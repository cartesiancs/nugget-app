...
export const characterGenApi = {
  ...
  startCharacterGeneration: async (characterData) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Character generation failed: ${errorData.message || response.statusText}`);
      }
  ...
  checkCharacterStatus: async (characterId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/character-gen/${characterId}`, {
        method: "GET",
        headers: await getAuthHeaders(),
      });
  ...
  generateVideoFromCharacter: async (characterId, videoConfig) => {
    try {
      const response = await fetch(`${API_BASE_URL}/character-gen/${characterId}/generate-video`, {
        method: "POST",
        headers: await getAuthHeaders(),
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(`Video generation failed: ${errorData.message || response.statusText}`);
      }
