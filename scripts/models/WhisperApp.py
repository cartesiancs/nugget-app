...
# Define constants that were imported from qai_hub_models
CHUNK_LENGTH = 30
HOP_LENGTH = 160
N_FFT = 400
N_MELS = 80
...
class WhisperApp:
    ...
    def __init__(
        self,
        whisper,  # Changed from whisper: Whisper since Whisper class is not available
        ...
        if not self.mel_filter:
            # Create a simple mel filter using librosa since MEL_FILTER_PATH is not available
            import librosa
...
def log_mel_spectrogram(
    ...
    mel_spec = torch.from_numpy(mel_filter).float() @ magnitudes.float()
